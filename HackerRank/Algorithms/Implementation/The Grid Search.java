...
public class Solution {
    ...
    static void checkGrid(String[] large, String[] small) {
        // Number of founded rows of small grid
        int sml_row = 0;
        
        if (large.length <= 1) {
            System.out.println(getMatchIndex(large[0], small[0]) != -1 ? "YES" : "NO");
            return;
        }
        
        for(int lar_row = 0; lar_row < large.length - 1; lar_row++) {
            if ((sml_row >= small.length - 1) 
                || (((large.length - lar_row) < small.length) 
                        &&  sml_row == 0)){
                // Check Valid, if not break
                // If either find all digits in small grid 
                // or the number of remaining unexaminated rows of
                // the larger grid is less than the number of rows
                // of the smaller grid and sml_row == 0, then exit the loop
                break;
            }
            
            int indexOfCurrentLine = getMatchIndex(large[lar_row], small[sml_row]);
            int indexOfNextLine = getMatchIndex(large[lar_row + 1], small[sml_row + 1]);
            if (indexOfCurrentLine == indexOfNextLine
                && indexOfCurrentLine != -1) {
                // If match index of 2 continuous lines is the same and not equal -1, then found it
                sml_row++;
            } else {
                sml_row = 0;
            }
        ...
        System.out.println(sml_row > 0 ? "YES" : "NO");
                
    }
    
    static int getMatchIndex(String largeString, String smallString) {
        return largeString.indexOf(smallString);
    }
}
